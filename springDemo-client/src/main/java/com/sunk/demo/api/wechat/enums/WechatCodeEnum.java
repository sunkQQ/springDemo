package com.sunk.demo.api.wechat.enums;
...
public enum WechatCodeEnum {
    ...
    code_40033(40033, "不合法的请求字符，不能包含uxxxx格式的字符"),
    ...
    private static final Map<Integer, WechatCodeEnum> WECHAT_CODE_MAP = new HashMap<>(200);

    static {
        for (WechatCodeEnum thisEnum : WechatCodeEnum.values()) {
            WECHAT_CODE_MAP.put(thisEnum.getCode(), thisEnum);
        }
    }
    ...
    WechatCodeEnum(Integer code, String messange) {
        this.code = code;
        this.messange = messange;
    }

    public static WechatCodeEnum getEnum(Integer code) {
        return WECHAT_CODE_MAP.get(code);
    }

    public static String getMessange(Integer code) {
        WechatCodeEnum thisEnum = getEnum(code);
        if (thisEnum != null) {
            return thisEnum.getMessange();
        }
        return null;
    }
